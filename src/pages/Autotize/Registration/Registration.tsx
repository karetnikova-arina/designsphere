...
export function Registration() {
    ...
    const formatName = (nameString: string) => {
        return `${nameString.length > 0 ? nameString[0].toUpperCase() : ""}${nameString.length > 1 ? nameString.slice(1) : ""}`;
    };
                ...
                <div className={style.radioButtons}>
                    <AuthElement title="Кто вы?">
                        <div className={style.radio}>
                            <RadioButton title="Дизайнер" setValue={setPerson} chosen={checkedPerson === "Дизайнер"} />
                            <RadioButton title="Работодатель" setValue={setPerson} chosen={checkedPerson === "Работодатель"} />
                        </div>
                        {!validation && <ErrorMessage text="Поле не заполнено"/>}
                    </AuthElement>
                </div>
                <AuthElement title="Имя">
                    <Input isValid={isValid.name} value={formatName(values.name)}
                           onChange={(e) => changeValue(e)} name="name"
                           placeholder="Иван"/>
                </AuthElement>
                <AuthElement title="Фамилия">
                    <Input isValid={isValid.surname} value={formatName(values.surname)}
                           onChange={(e) => changeValue(e)} name="surname"
                           placeholder="Иванов"/>
                </AuthElement>
                <AuthElement title="Никнейм">
                    <Input isValid={isValid.nikname} value={formatNikname(values.nikname)}
                           onChange={(e) => changeValue(e)} name="nikname"
                           placeholder="@ivan.ivanov"/>
                </AuthElement>
                <AuthElement title="Электронная почта">
                    <Input isValid={isValid.email} value={values.email}
                           onChange={(e) => changeValue(e)} name="email" type="email"
                           placeholder="ivan123@mail.ru"/>
                </AuthElement>
                <AuthElement title="Номер телефона">
                    <InputPhone isValid={isValid.tel} value={values.tel}
                                onChange={changeValue} name="tel" type="tel"
                                placeholder="+7 (999) 999-99-99"/>
                </AuthElement>
                <Button isValid={checkedPerson.length !== 0 && isValid.name && isValid.tel && isValid.surname && isValid.nikname && isValid.email}>
                    Далее
                </Button>
            </form>
