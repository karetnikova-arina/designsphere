    ...
    return(
        <div className={style.inputSearch}>
            ...
            <div className={style.element}>
                <button onClick={()=>setFilters(prevState => !prevState)} className={cn(style.filter, {
                    [style.focusFilter]: inputFocus
                })}>
                    <div>{location.pathname.includes("group") ? "Фильтр" :"Параметры"}</div>
                    <svg className={cn(style.arrowTransform, {[style.arrow]: filters})} width="20" height="11" viewBox="0 0 20 11" fill="none"
